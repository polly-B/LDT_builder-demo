...
for thisPractice in practice:
    ...
    itemR.setText(right)
    buttonBoxButton = []  # to store response values
    buttonBoxRT = []
    # keep track of which components have finished
            ...
            if useButtonBox:
                if t >= 1: 
                    ...
                    if response != ['1234']:
                            continue
    #                if response == ['123']:
    #                      buttonBox.clock.getTime()
    #                buttonBox.reset_input_buffer()  # don't process again
                    if len(response) > 0:  # at least one key was pressed
                        if buttonBox.response == []:  # then this is first keypress
                            buttonBox.response = theseKeys[0]  # the first key pressed
                            buttonBox.timing = theseRTs[0]
                            # was this 'correct'?
                        if (buttonBox.response == str(corrAns)) or (buttonBox.response == corrAns):
                            buttonBox.corr = 1
                        else:
                            buttonBox.corr = 0
                        # a response ends the routine
