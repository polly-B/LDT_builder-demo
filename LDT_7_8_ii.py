...
def initialiseButtonBox():
    ...
    print('line is', line)
    if line == 'Initialised\r\n':
        print('buttonBox', line.split('\r')[0])
        buttonBox.write(b'p')
        line = buttonBox.readline()
...
for thisPractice in practice:
    ...
    while continueRoutine:
        ...
        if t >= 0.5 and fixation.status == NOT_STARTED:
            # keep track of start time/frame for later
             if useButtonBox and not triggerTest:
                ...
                fixation.setAutoDraw(True);buttonBoxStimuli.draw()
             else:fixation.tStart = t
             fixation.frameNStart = frameN  # exact frame index
        ...
        if resp.status == STARTED:
            ...
            if useButtonBox and not triggerTest:
                buttonBox.write() #start looking for a flash of light to trigger the clock
            elif useButtonBox and triggerTest:
                 buttonBox.write() #if we're testing the trigger. Run the trigger test. Outputs "Triggered\r\n" if the photo cell is triggered
        if useButtonBox and not triggerTest:
